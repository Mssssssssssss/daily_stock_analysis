...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.notification.WECOM_STREAM_ENABLED': {
    title: '企业微信长连接机器人',
    summary: '启用企业微信智能机器人长连接；运行 python main.py --serve 时会启动后台 WebSocket worker。',
    usage: '配置 WECOM_STREAM_BOT_ID 和 WECOM_STREAM_CLIENT_SECRET 后启用；WECOM_STREAM_CLIENT_ID 仅作为旧配置 BotID fallback。',
    valueNotes: [
      'true 会随服务连接企业微信长连接，不会启动 /bot/wecom HTTP endpoint。',
      'false 保持默认无影响；企业微信群报告推送仍使用 WECHAT_WEBHOOK_URL。',
    ],
    impact: [
      '支持 /chat、/ask、/help；群聊 @机器人 的非命令文本和单聊纯文本会进入 /chat。',
      '不影响通知测试、企业微信 Webhook 推送或主分析流程。',
    ],
    notes: [
      '保存后需要重启相关 bot/服务进程，已运行的长连接不会自动重建。',
      '不要把该开关当作 WECHAT_WEBHOOK_URL 的替代配置。',
    ],
  },
  'settings.notification.WECOM_STREAM_CLIENT_ID': {
    title: '企业微信长连接 Client ID',
    summary: '兼容旧配置的 BotID fallback。',
    usage: '新配置优先填写 WECOM_STREAM_BOT_ID；仅在 Bot ID 为空时读取该字段作为 BotID。',
    valueNotes: ['该值按敏感信息处理，Web 读取时会被掩码。'],
    impact: ['不影响企业微信 Webhook 推送、通知测试或分析流程。'],
    notes: ['迁移后建议改用 WECOM_STREAM_BOT_ID。'],
  },
  'settings.notification.WECOM_STREAM_CLIENT_SECRET': {
    title: '企业微信长连接 Client Secret',
    summary: '企业微信智能机器人长连接 Secret。',
    usage: '从企业微信后台 API 模式长连接配置中复制 Secret；订阅时会作为 aibot_subscribe body.secret 发送。',
    valueNotes: ['该值按敏感信息处理，Web 读取时会被掩码。'],
    impact: ['不影响企业微信 Webhook 推送、通知测试或分析流程。'],
    notes: ['不要提交到版本库。'],
  },
  'settings.notification.WECOM_STREAM_BOT_ID': {
    title: '企业微信长连接 Bot ID',
    summary: '企业微信智能机器人长连接 BotID。',
    usage: '从企业微信后台 API 模式长连接配置中复制 BotID；订阅时会作为 aibot_subscribe body.bot_id 发送。',
    valueNotes: ['推荐新配置填写该字段。'],
    impact: ['不影响企业微信 Webhook 推送、通知测试或分析流程。'],
    notes: ['WECOM_STREAM_BOT_ID 为空时才会 fallback 到 WECOM_STREAM_CLIENT_ID。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.notification.WECOM_STREAM_ENABLED': {
    title: 'WeCom Stream Bot',
    summary: 'Enable the Enterprise WeChat intelligent bot long connection. python main.py --serve starts the background WebSocket worker.',
    usage: 'Set WECOM_STREAM_BOT_ID and WECOM_STREAM_CLIENT_SECRET before enabling it; WECOM_STREAM_CLIENT_ID is only a legacy BotID fallback.',
    valueNotes: [
      'true connects to the WeChat Work long connection with the service. It does not start a /bot/wecom HTTP endpoint.',
      'false is the no-op default. Enterprise WeChat group report delivery still uses WECHAT_WEBHOOK_URL.',
    ],
    impact: [
      'Supports /chat, /ask, and /help. Mentioned group plain text and private plain text are routed to /chat.',
      'Does not affect notification tests, Enterprise WeChat webhook delivery, or the main analysis flow.',
    ],
    notes: [
      'Restart the relevant bot/service process after saving; existing long connections are not rebuilt automatically.',
      'Do not treat this switch as a replacement for WECHAT_WEBHOOK_URL.',
    ],
  },
  'settings.notification.WECOM_STREAM_CLIENT_ID': {
    title: 'WeCom Stream Client ID',
    summary: 'Legacy BotID fallback for older configs.',
    usage: 'Prefer WECOM_STREAM_BOT_ID for new configs. This field is read only when Bot ID is empty.',
    valueNotes: ['This value is sensitive and is masked when read by the Web settings page.'],
    impact: ['Does not affect Enterprise WeChat webhook delivery, notification tests, or analysis.'],
    notes: ['After migration, prefer WECOM_STREAM_BOT_ID.'],
  },
  'settings.notification.WECOM_STREAM_CLIENT_SECRET': {
    title: 'WeCom Stream Client Secret',
    summary: 'Enterprise WeChat intelligent bot long-connection Secret.',
    usage: 'Paste the Secret from the Enterprise WeChat API-mode long-connection setup. It is sent as aibot_subscribe body.secret.',
    valueNotes: ['This value is sensitive and is masked when read by the Web settings page.'],
    impact: ['Does not affect Enterprise WeChat webhook delivery, notification tests, or analysis.'],
    notes: ['Do not commit it to version control.'],
  },
  'settings.notification.WECOM_STREAM_BOT_ID': {
    title: 'WeCom Stream Bot ID',
    summary: 'Enterprise WeChat intelligent bot long-connection BotID.',
    usage: 'Paste the BotID from the Enterprise WeChat API-mode long-connection setup. It is sent as aibot_subscribe body.bot_id.',
    valueNotes: ['Recommended for new configs.'],
    impact: ['Does not affect Enterprise WeChat webhook delivery, notification tests, or analysis.'],
    notes: ['Only when WECOM_STREAM_BOT_ID is empty does runtime fall back to WECOM_STREAM_CLIENT_ID.'],
  },
  'settings.notification.webhooks': {
