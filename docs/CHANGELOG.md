...
- [修复] Agent 系统提示词明确区分市场自然日及其星期与最近完整日线日期，避免 A 股盘中将上一交易日误称为“今天”。
...
- [新功能] 接入企业微信智能机器人长连接（`WECOM_STREAM_*`），`python main.py --serve` 可主动连接 WebSocket 并复用 `/chat`、`/ask`、`/help` 命令，不改变企业微信 Webhook 推送行为。
