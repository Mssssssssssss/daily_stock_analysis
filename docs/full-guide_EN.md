...
Agent API, Bot, and direct single-/multi-Agent entrypoints also freeze a server-owned market-local time and latest complete daily-bar date at the start of every request; same-named client `context` fields cannot replace it. In conversation, “today” means the security market's natural date (market-wide questions without an explicit market default to A-shares), and daily-bar conclusions may only use that frozen complete bar. Realtime, history, and market tools return fetch time, source, and provider time when available. Data with missing or stale freshness may be used only as historical reference: the answer must state its as-of date/fetch time and limitation, and must not call it “today's realtime quote.”
